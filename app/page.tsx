...
export default function HomePage({
  ...
  const jsonLd: Record<string, unknown> = {
    ...
    url: 'https://toolo.cn/',
    description:
      '在线工具箱：图片压缩、图片格式转换、GIF 压缩、视频转 GIF、SRT 字幕清洗、字幕时间轴偏移、字幕合并拆分。默认浏览器本地处理，不上传，无需登录。',
    potentialAction: {
      '@type': 'SearchAction',
  ...
  const hotTools: ToolItem[] = [
    ...
    {
      title: 'GIF 压缩',
      description: '缩放 / 抽帧 / 降色三旋钮，本地处理；支持单个下载与下载全部（逐个触发）。',
      href: '/gif/compress',
      badge: '可用',
      tone: 'violet',
      icon: <IconGif />,
    ...
    {
      title: 'SRT 清洗/格式化',
      description: '一键规范字幕内容，清理空行与异常格式，便于剪辑/翻译流程。',
    ...
    {
      title: '字幕时间轴偏移',
      description: '整体前移/后移字幕时间轴，快速对齐音画；本地处理不上传。',
    ...
    {
      title: '字幕合并/拆分',
      description: '按段合并或拆分字幕文件，适合多语言/多段导出场景。',
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white/70 backdrop-blur-sm px-6 sm:px-10 py-10 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1">
          ...
          <p className="text-xs text-slate-500">
            目标明确：直接用下方“热门工具”。不确定从哪开始：从“图片压缩”通常最省时间。
          </p>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-600">
              直接点卡片进入。也可用搜索框快速筛选（支持 URL 参数 <span className="font-medium">?q=</span>）。
            </p>
          </div>
          ...
          <div className="w-full sm:w-[360px]">
            ...
            <input
              ...
              inputMode="search"
              defaultValue={q}
              placeholder="搜索：压缩 / 转换 / GIF / SRT / 偏移…"
        ...
        <p id="site-search-empty-home" className="hidden text-sm text-slate-600">
          没有匹配的工具。你可以换个关键词（例如：压缩 / WebP / SRT / 偏移）。
        </p>
  ...
  apply(); // 初次渲染：支持 ?q= 预填后立刻筛选
      ...
      <section className="rounded-3xl border border-slate-200 bg-white px-6 sm:px-10 py-8 space-y-3">
        ...
        <p className="text-sm text-slate-600 leading-relaxed max-w-3xl">
          toolo.cn 一期默认“浏览器本地处理”。你选择的文件（例如 .srt、图片、GIF、视频片段）优先在本机完成计算，
