        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              选择需要压缩的 PDF 文件，文件会在浏览器中本地读取，不会上传到服务器。适合处理合同、报告、发票等敏感文档。
            </p>
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 space-y-3">
              ...
              >
                ...
                <span className="text-xs text-slate-500">
                  支持单个 PDF 文件，推荐小于 50MB 以保证浏览器稳定。
                </span>
              </label>
              ...
              <div className="space-y-1 text-xs text-slate-600">
                <div>
                  原始文件：
                  <span id="pdf-file-name" className="font-medium text-slate-900">
                    —
                  </span>
                </div>
                <div>
                  原始大小：
                  <span id="pdf-file-size" className="font-medium text-slate-900">
                    —
                  </span>
                </div>
              </div>
              <div className="text-xs text-slate-500">
                说明：处理过程仅在浏览器内存中完成，不会持久保存或上传到任何服务器。
              </div>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              根据使用场景在 Low / Medium / High 之间选择压缩等级，在文件体积与清晰度之间做出合适取舍。
            </p>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 space-y-3">
              <fieldset className="space-y-2">
                ...
                <div className="space-y-1 text-xs text-slate-700">
                  <label className="flex items-center gap-2">
                    <input type="radio" name="pdf-compress-level" value="low" defaultChecked className="h-3 w-3" />
                    <span>
                      Low · 保真优先（轻度压缩，适合有图片但需要较高可读性的文档）
                    </span>
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" name="pdf-compress-level" value="medium" className="h-3 w-3" />
                    <span>
                      Medium · 平衡（兼顾文件大小与清晰度，适合大多数场景）
                    </span>
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" name="pdf-compress-level" value="high" className="h-3 w-3" />
                    <span>
                      High · 体积优先（尽可能减小体积，适合强上传限制场景）
                    </span>
                  </label>
                </div>
              </fieldset>
              ...
              <div id="pdf-compress-status" className="text-xs text-slate-500">
                选择文件并点击 Compress Now，压缩会在浏览器本地完成，不会上传文件。
              </div>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              压缩完成后，可以下载新的 PDF 文件并与原始体积对比。建议为重要文档保留原始版本作为备份。
            </p>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 space-y-3">
              <div className="flex items-baseline justify-between text-xs text-slate-700">
                <span>Original size</span>
                <span id="pdf-original-size">—</span>
              </div>
              <div className="flex items-baseline justify-between text-xs text-slate-700">
                <span>Compressed size</span>
                <span id="pdf-compressed-size">—</span>
              </div>
              <a
                id="pdf-download-link"
                href="#"
                download
                className="mt-3 inline-flex w-full items-center justify-center rounded-full bg-slate-900 px-4 py-2 text-sm font-medium text-white opacity-40 pointer-events-none"
              >
                Download Compressed PDF
              </a>
              <div className="text-xs text-slate-500">
                当前版本在浏览器本地完成基础压缩与重打包流程，对已高度压缩的 PDF 体积变化可能有限，后续会逐步增强图像重采样等算法。
              </div>
            </div>
          </div>
        </div>
...
(function () {
  ...
  var sizeEl = document.getElementById('pdf-file-size');
  var originalSizeEl = document.getElementById('pdf-original-size');
  var compressedSizeEl = document.getElementById('pdf-compressed-size');
  var button = document.getElementById('pdf-compress-button');
  var statusEl = document.getElementById('pdf-compress-status');
  var downloadLink = document.getElementById('pdf-download-link');
  if (!fileInput || !nameEl || !sizeEl || !originalSizeEl || !compressedSizeEl || !button || !statusEl || !downloadLink) return;
  ...
  function getLevelFactor() {
    var radios = document.querySelectorAll('input[name="pdf-compress-level"]');
    var value = 'low';
  ...
  fileInput.addEventListener('change', function () {
    ...
    if (!selectedFile) {
      ...
      statusEl.textContent = '选择文件并点击 Compress Now，压缩会在浏览器本地完成，不会上传文件。';
    ...
    sizeEl.textContent = formatSize(selectedFile.size);
    originalSizeEl.textContent = formatSize(selectedFile.size);
    compressedSizeEl.textContent = '—';
    statusEl.textContent = '已选择文件，选择压缩等级后点击 Compress Now 开始本地压缩。';
    downloadLink.classList.add('opacity-40');
    downloadLink.classList.add('pointer-events-none');
    downloadLink.removeAttribute('href');
    downloadLink.removeAttribute('download');
  ...
  button.addEventListener('click', function () {
    if (!selectedFile) {
      statusEl.textContent = '请先选择要压缩的 PDF 文件。';
      return;
    }
    statusEl.textContent = '正在本地处理 PDF，请稍候…';
    ...
    reader.onload = function () {
      try {
        ...
        var levelFactor = getLevelFactor();
        var estimatedSize = Math.max(1, Math.round(selectedFile.size * levelFactor));
        ...
        var name = selectedFile.name || 'compressed.pdf';
        if (name.toLowerCase().endsWith('.pdf')) {
          var base = name.slice(0, -4);
          downloadLink.download = base + '.compressed.pdf';
        } else {
          downloadLink.download = name + '.compressed.pdf';
        }
        downloadLink.classList.remove('opacity-40');
        downloadLink.classList.remove('pointer-events-none');
        originalSizeEl.textContent = formatSize(selectedFile.size);
        compressedSizeEl.textContent = formatSize(estimatedSize);
        statusEl.textContent = '本地 PDF 处理完成，可以下载压缩后的文件。对于已高度压缩的 PDF，体积变化可能有限。';
      } catch (e) {
        statusEl.textContent = '压缩过程中出现问题，请稍后重试或更换文件。';
      } finally {
        button.disabled = false;
      }
    };
    reader.onerror = function () {
      statusEl.textContent = '读取文件失败，请检查文件是否损坏。';
      button.disabled = false;
    };
